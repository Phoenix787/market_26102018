...
public class OrderCardHeaderGenerator {
    ...
    private final DateTimeFormatter HEADER_DATE_TIME_FORMATTER = DateTimeFormatter.ofPattern("EEE, d MMMM");
    ...
    private OrderCardHeader getRecentHeader() {
        return new OrderCardHeader("Последние", "До этой недели");
    }

    private OrderCardHeader getYesterdayHeader() {
        LocalDate yesterday = LocalDate.now().minusDays(1);
        return new OrderCardHeader("Вчера", secondaryHeaderFor(yesterday));
    }

    private OrderCardHeader getTodayHeader() {
        LocalDate today = LocalDate.now();
        return new OrderCardHeader("Сегодня", secondaryHeaderFor(today));
    }
    ...
    private OrderCardHeader getThisWeekStartingTomorrow(boolean showPrevious) {
        ...
        return new OrderCardHeader(showPrevious ? "На этой неделе, начиная с завтрашнего дня" : "Эта неделя",
                secondaryHeaderFor(tomorrow, nextWeekStart));
    }

    private OrderCardHeader getUpcomingHeader() {
        return new OrderCardHeader("Предстоящие", "После этой недели");
    }
